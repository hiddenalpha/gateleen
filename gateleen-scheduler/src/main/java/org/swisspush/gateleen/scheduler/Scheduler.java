...
public class Scheduler {
    ...
    private void trigger() {
        for (final HttpRequest request : requests) {
            ...
            var enqueOp = buildEnqueueOperation(queueName, request.toJsonObject().put(QueueClient.QUEUE_TIMESTAMP, System.currentTimeMillis()).encode());
            vertx.eventBus().request(redisquesAddress, enqueOp, (Handler<AsyncResult<Message<JsonObject>>>) event -> {
                if( event.failed() ){
                    if( log.isWarnEnabled() ) log.warn("redisques.enqueue({}, {})", redisquesAddress, queueName, new Exception(event.cause()));
                    return;
                }
